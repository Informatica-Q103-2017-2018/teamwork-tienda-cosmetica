...
void main(){
	...
	producto datosM[3]={ {"1 pintalabios",20,1}, {"2 colorete",18,2}, {"3 mascara de pestañas",25,3} };
	...
	printf("¡Bienvenido a nuestra tienda de cosmetica!\n\n");
	...
	do{
		...
		switch(menu){
			...
			case 'P':
				do{
					...
					switch(categoria){                        
						...
						case'm':
						case 'M':
							printf(" A continuación, se muestra una lista de los productos disponibles\n\n");   
							for(i=0;i<3;i++){
								printf("%s\n",datosM[i].nombre);
							}
							printf("\n");
							printf("Si desea comprar algun producto pulse 1, sino saldra de la aplicacion automaticamente \n\n"); 
							scanf("%d", &tecla);
					
							if(tecla==1){
								Compra(datosM);
								printf("Su pedido se enviara a la direccion registrada \n\n");
							}
			...
			case 'R':
				...
				if(f==NULL){
					printf("Lo sentimos. No se ha podido realizar la reclamación\n\n");
					break;
				}
...
void Registro(contacto c1) {
	...
	cuenta personas[1]={"oscar","1234"};
		...
		do {                                        //compara el usuario introducido con el usuario que tenemos guardado, en caso de que coincidan lo volverá a pedir
		...
		do{
			...
			printf("Crea tu contraseña: ");
			scanf("%s", c1.password1);
				while(strlen(c1.password1)>8){        //La contraseña no puede tener mas de ocho caracteres, si los tiene la volverá a pedir.
					printf("La contraseña es demasiado larga, vuelva a introducirla:\n");
					scanf("%s", c1.password1);
					}
			...
			orden1 = strcmp(c1.password1, c1.password2);  //compara las dos contraseñas introducidas 
			if(orden1!=0){
				printf("Las contraseñas no coinciden, pruebe de nuevo por favor\n.\n");
			}
